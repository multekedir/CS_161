...
} bedRooom, masterBedRoom, livingRoom, kitchen, dining, laundry, garage, porch,
		lHall, door, westBHall, eastBHall, wall;
...
int main() {
	...
	cout
			...
			<< endl
			<< "Your objective for this game is to find the key and leave the house!"
			<< endl
			<< "Make sure you do this before getting hungry, food can be found in the kitchen"
			<< endl;

	cout << "Your list of commands are" << endl << "\t  0  \t go north" << endl
			<< "\t  1  \t go south" << endl << "\t  2  \t go west" << endl
			<< "\t  3  \t go east" << endl << "\t  4  \t to peek next room"
			<< endl << "\t  5  \t check items" << endl << "\t  6  \t open door"
			<< endl << "\t  7  \t quit" << endl;
	do {
...
bool get(Item item, Room * curent) {
	...
	if (item.name == key.name && curent->name == porch.name) {
		...
		cout << "You Picked up Tide Pod" << endl
				<< "Do you want to the Challenge? Yes(y)/ No(n)";
		pod.pickedItem = true;
		doPodchallenge();
		return true;
	}
